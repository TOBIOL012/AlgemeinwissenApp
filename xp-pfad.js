document.addEventListener("DOMContentLoaded", function () {
  ...
  function initXPAndCheckpoints() {
    ...
    // Checkpoints auf 30 gesetzt
    const checkpointCount = 30;
    const step = totalLength / (checkpointCount + 2);
    const checkpointSize = 70;
    const borderRadius = 30;

    // Ursprüngliches Belohnungs-Array (10 Elemente)
    const originalRewards = [
      { xpNeeded: 100, type: "münzen", value: 50, icon: "coin.png" },
      { xpNeeded: 250, type: "münzen", value: 100, icon: "coin.png" },
      { xpNeeded: 500, type: "profilbild", value: "neues-profilbild", icon: "/Profilbilder/Albert Einstein.png" },
      { xpNeeded: 750, type: "streak", value: 1, icon: "streak.png" },
      { xpNeeded: 1000, type: "münzen", value: 200, icon: "coin.png" },
      { xpNeeded: 1500, type: "münzen", value: 150, icon: "coin.png" },
      { xpNeeded: 2000, type: "profilbild", value: "besonderes-bild", icon: "/Profilbilder/Donald Trump.png" },
      { xpNeeded: 2500, type: "streak", value: 2, icon: "streak.png" },
      { xpNeeded: 3000, type: "münzen", value: 300, icon: "coin.png" },
      { xpNeeded: 3500, type: "münzen", value: 500, icon: "coin.png" }
    ];
    // Für 30 Checkpoints (Start-Checkpoint plus 29 Belohnungen) wird das Array wiederholt.
    const belohnungen = [];
    for (let i = 0; i < checkpointCount - 1; i++) {
      belohnungen.push(originalRewards[i % originalRewards.length]);
    }
    ...
    function berechneProzent(x) {
      ...
      if (x > 100) {
        prozent += Math.min(250, x - 100) * 3.3333333334;
      }
      if (x > 250) {
        prozent += Math.min(1000, x - 250) * 2;
      }
      if (x > 1000) {
        prozent += (x - 1000) * 1;
      }
    ...
    function start(XP) {
      checkpoints.forEach(checkpoint => {
        if (Math.min((XP / 18000) * totalLength, totalLength) >= checkpoint.position) {
    ...
    function fetchXP() {
      function scrollAfterAllData() {
        ...
        const progressLength = Math.min((localStorage.getItem("lastxp") / 18000) * totalLength, totalLength);
        progressPath.style.strokeDasharray = `${progressLength}, ${totalLength}`;
        ...
        const totalHeight = Math.min((umgerechnetxp / 18000) * totalLength, totalLength);
    ...
    window.updatexpbar = function () {
      const umgerechnetxp = berechneProzent(xp);
      updateXPProgress(umgerechnetxp, 18000);
    };
